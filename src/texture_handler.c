	...
	unsigned int *dst_height)
{
	...
	Some BMP files are misformatted, guess missing information if (imageSize == 0)
...
bool load_texture(t_app *app)
{
	...
	Check if texture file is provided as one of executable's arguments if (!app->texture_file_name) return true;
	...
	Load image, create texture and generate mipmaps if (import_image(
	...
	Tell shader that the texture object it must use has ID 0 set_uniform_int(app->shader_program, "texture1", 0);
	return (true);
}
